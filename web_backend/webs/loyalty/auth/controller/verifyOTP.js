...
export const verifyOTP = async (req, res, next) => {
    ...
    try {
        const user = await prisma.sfa_user.findUnique({ where: { contact_01, status: true } });

        if (!user) {
            return res.status(404).json({ status: false, message: "You are not allowed to Login In! " });
        }
        ...
        const data = await prisma.assign_designation_module.findMany({
            where: {
                designation_id: Number(user.designation_id),
                del: false
            },
        });

        return res.status(200).json({ message: 'OTP VERIFIED!', accessToken, refreshToken, data });
