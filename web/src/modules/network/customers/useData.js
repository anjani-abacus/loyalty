...
export const useDealers = () => {
  const { mutate, mutateAsync, data, error, isPending, isSuccess, reset } = useMutation({
    mutationFn: (params) => fetchDealers(params)
  })
  return { 
    fetchDealersData:mutate, 
    mutateAsync, dealersData:data?.dealer, 
  totalCount:data?.count, 
...
const dealerDetail = {
  ...
  distributorName: { label: 'Dealer Mobile', value: data?.data?.distributor_name },
}



return { detail:data?.data, isLoading, refetch, basicDetail, documentDetail, dealerDetail, isFetching }
};
